...
if [ -n "$GO_APPNAME_ENABLED" ]; then
  if [ "$GO_APPNAME_ENABLED" != "yes" ]; then
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
export PATH="/go/bin:/usr/local/etc/docker/bin:/usr/local/bin:/usr/bin:/usr/sbin:/bin:/sbin"
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
# command to execute
# Go is a CLI, not a daemon. Leave EXEC_CMD_BIN empty so the init pipeline
# finishes after __post_execute and the container idles via tini+entrypoint.
EXEC_CMD_BIN=''
# command arguments
EXEC_CMD_ARGS=''
...
# root user name
root_user_name="${GO_ROOT_USER_NAME:-}"
# root user password
root_user_pass="${GO_ROOT_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Normal user info [password/random]
# normal user name
user_name="${GO_USER_NAME:-}"
# normal user password
user_pass="${GO_USER_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Load variables from config
# Generated by my dockermgr script
if [ -f "/config/env/go.script.sh" ]; then
  . "/config/env/go.script.sh"
fi
# Overwrite the variables
if [ -f "/config/env/go.sh" ]; then
  . "/config/env/go.sh"
fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
ADD_APPLICATION_DIRS="/data/build /app /work /root/app /root/project /usr/local/share/go /usr/local/share/go/bin /usr/local/share/go/cache /usr/local/share/go/pkg/mod"
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
__run_precopy() {
  ...
  # Point /data/go at the canonical persistent Go state dir. Done at
  # runtime because /data is a volume mount and only exists once the
  # container starts. Skip if /data/go is already a real, populated dir
  # (legacy upgrade path - leave the user's data alone).
  mkdir -p /usr/local/share/go
  if [ ! -e /data/go ] || [ -L /data/go ]; then
    mkdir -p /data
    ln -sfn /usr/local/share/go /data/go
  fi
...
__post_execute() {
  ...
  pid=$!
  if ps ax | awk -v pid="$execPid" '$1 == pid {found=1} END {exit !found}'; then
    retVal=0
  else
    retVal=10
  fi
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$GO_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$GO_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$GO_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$GO_USER_PASS_WORD}"             #
